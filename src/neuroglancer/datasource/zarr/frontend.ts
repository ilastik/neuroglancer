...
/**
 * Supports single-resolution and multi-resolution zarr datasets
 *
  ...
  constructor(obj: any) {
    ...
    if (encoding === undefined) {
      encoding = verifyObjectProperty(
          obj, 'compressor', () => VolumeChunkEncoding.RAW);
    }
...
function getAttributesJsonUrls(url: string): string[] {
  ...
  const urls: string[] = [];
  while (true) {
    urls.push(`${protocol}://${host}${path}/.zarray`);
...
function getMultiscaleMetadata(url: string, attributes: any): MultiscaleMetadata {
  ...
  const dimensions = verifyOptionalObjectProperty(attributes, 'shape', x => {
